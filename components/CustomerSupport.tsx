...
type Metadata = { title: string; description: string; robots?: { index?: boolean; follow?: boolean }; metadataBase?: { href: string }; alternates?: { canonical: string }; openGraph?: { title?: string; description?: string; url?: string; type?: string; siteName?: string; images?: { url: string; width: number; height: number; alt: string }[] }; twitter?: { card?: string; title?: string; description?: string; images?: string[] } };
type ContentData = { hero: HeroSectionData; introduction: string; services: ServicesSectionData; approach: ApproachSectionData; faqs: FAQsSectionData; whyChoose: WhyChooseData; metadata?: Metadata; script?: any };

export default function CustomSoftwareDevelopment() {
  const service = "customer-support";

  const fetchServiceData = async (): Promise<ContentData> => {
    const response = await axios.post(`${baseURL}/service`, { name: service });
